...
# ! Обновлять расписание через бота: последняя фича админа
# ? 1. Создать 7 документов с днями недели
...
#? Какой день будем редактировать? (генерирует кнопки дней недели) (берёт их из БД??) 
#? Понедельник (id = 0)
#? Вторник (id = 1)
#? Среда
#? ...

#? Отправляет мне текст: Вот уроки этого дня (из БД) | Или текста нет, тогда пишет, что на этот день ещё нет расписания... И предлагает ввести текст

# ? Если текст в БД есть, я копирую его из смс бота, всталяю его, правлю текст на новый и отправляю боту на сохранение

# ? Тут, если нужно, будет валидация текста 
# ? Бот сохраняет расписание в бд... 
# ? Новое расписание уже в боте! Смотри /schedule
